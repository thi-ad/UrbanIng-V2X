...
# copied from cobest (not mean, std for our images)
MEAN = np.array([0.485, 0.456, 0.406])
STD = np.array([0.229, 0.224, 0.225])


def load_specific_epoch_model(saved_path: str, model, epoch: int):
    assert os.path.exists(saved_path), '{} not found'.format(saved_path)
    
    model_file_path = os.path.join(saved_path, f'net_epoch{epoch}.pth')
    assert os.path.exists(model_file_path)
    
    print(f'Loading weights of epoch {epoch}')
    
    model.load_state_dict(torch.load(model_file_path, map_location='cpu'), strict=False)
    
    return model


def load_saved_model(saved_path, model, load_best: bool = False):
    """
    ...
    if load_best and os.path.exists(os.path.join(saved_path, 'best_model.pth')):
        initial_epoch = 'best'
        model_file = os.path.join(saved_path, 'best_model.pth')
        print('resuming by loading best model')
    ...
    # model_file = os.path.join(saved_path,
    #             'net_epoch%d.pth' % 70) \
    # if 70 != 10000 else os.path.join(saved_path,
    #                 'latest.pth')
    # print('resuming by loading epoch %d' % 70)
    # checkpoint = torch.load(
    #     model_file,
    #     map_location='cpu')
    # model.load_state_dict(checkpoint, strict=False)
...
def setup_train(hypes, is_camera=False):
    """
    ...
    current_path = os.path.join(current_path, '../logs', 'camera' if is_camera else 'lidar')
...
def create_model(hypes):
    ...
    model = None
    target_model_name = backbone_name.replace('_', '').split('.')[-1]
...
def save_bev_seg_binary(output_dict,
    ...
    for i in range(batch_size):
        ...
        # (M, H, W, 3)
        # rows = batch_dict['ego']['inputs'].shape[0]
        cols = batch_dict['ego']['inputs'].shape[2]  # amount of cameras
        raw_images = \
            batch_dict['ego']['inputs'].detach().cpu().data.numpy()[index, 0]
        visualize_summary = np.zeros((raw_images[0].shape[0] * 2,
                                      raw_images[0].shape[1] * cols,
                                      3),
                                     dtype=np.uint8)
