...
class VanillaSegLoss(nn.Module):
    ...
    def forward(self, output_dict, gt_dict):
        ...
        Loss dictionary.
        """
        # weights of loss func to device
        self.loss_func_static.weight = \
            self.loss_func_static.weight.to(output_dict['static_seg'].device)
        self.loss_func_dynamic.weight = \
            self.loss_func_dynamic.weight.to(output_dict['dynamic_seg'].device)
