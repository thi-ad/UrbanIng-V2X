...
def filter_boxes_within_radius(boxes: torch.Tensor, radius: float, center: torch.Tensor = None):
    """
    Filters boxes such that at least one of the 8 corners lies within the given radius from a center point.
    
    Args:
        boxes: Tensor of shape (N, 8, 3)
        radius: float, radius threshold
        center: Tensor of shape (3,), center of the sphere (default [0, 0, 0])
    
    Returns:
        Filtered boxes: Tensor of shape (M, 8, 3), where M <= N
    """
    radius = float(radius)

    if center is None:
        center = torch.tensor([0.0, 0.0, 0.0], device=boxes.device)

    # Calculate squared distance of each corner to the center (to avoid sqrt for speed)
    dists_squared = ((boxes - center) ** 2).sum(dim=2)  # Shape: (N, 8)

    # Check if any corner of each box is within the squared radius
    mask = (dists_squared < radius ** 2).any(dim=1)  # Shape: (N,)

    # Apply mask to filter boxes
    return boxes[mask], mask


def filter_boxes_within_min_max_radius(boxes: torch.Tensor, min_radius: float, max_radius: float, center: torch.Tensor = None):
    """
    Filters boxes such that at least one of the 8 corners lies within the given min and max radius from a center point.
    
    Args:
        boxes: Tensor of shape (N, 8, 3)
        min_radius: float, minimum radius threshold
        max_radius: float, maximum radius threshold
        center: Tensor of shape (3,), center of the sphere (default [0, 0, 0])
    ...
    # Check if any corner of each box is within the squared radius
    mask = ((dists_squared > min_radius ** 2) & (dists_squared < max_radius ** 2)).any(dim=1)  # Shape: (N,)
